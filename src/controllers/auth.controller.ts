...
export class AuthController {
  ...
  public resendVerify = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const data: ResendVerifyDto = req.body;
    const userData: UserResponse = await this.auth.resendVerifyEmail(data);

    res.status(201).json(apiResponse(201, "OK", "Resend Verify Email Success", userData));
  });
