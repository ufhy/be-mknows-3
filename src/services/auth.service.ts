...
import { CreateUserDto, ResendVerifyDto } from "@dtos/users.dto";
...
export class AuthService {
  ...
  public async resendVerifyEmail(data: ResendVerifyDto): Promise<{ uuid: string, email: string }> {
    const existingUser = await DB.Users.findOne({ where: { email: data.email } });
    if (!existingUser) {
      throw new HttpException(false, 409, `This email ${data.email} not exists`);
    }

    const otpService = new OTPService();
    const transaction = await DB.sequelize.transaction();
    try {
      // clear OLD EMAIL_VERIFICATION OTP
      otpService.clearOTP(existingUser.pk);
      ...
      return { uuid: existingUser.uuid, email: existingUser.email };
    } catch (error) {
      await transaction.rollback();
      throw error; 
    }
